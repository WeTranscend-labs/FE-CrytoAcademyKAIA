...
export default function StepByStepEditor({
    ...
    const handleRunCode = async () => {
        ...
        try {
            const result = await validateSolution(code, currentStep.tests);
        ...
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="bg-white border-b px-4 py-3 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                    <span className="text-gray-600 text-sm">Step {currentStepIndex + 1} of {steps.length}</span>
                    <div className="flex space-x-1">
                        {steps.map((step, index) => (
                            <div
                                key={step.id}
                                className={`h-1.5 w-6 rounded ${index === currentStepIndex
                                    ? 'bg-blue-500'
                ...
                <div className="flex space-x-2">
                    <button
                        ...
                        className={`p-1 rounded ${isFirstStep ? 'text-gray-300' : 'text-gray-600 hover:bg-gray-100'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`p-1 rounded ${isLastStep || !completedSteps.has(currentStep.id)
                            ? 'text-gray-300'
